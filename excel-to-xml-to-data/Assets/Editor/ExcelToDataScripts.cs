...
public class ExcelToDataScripts {

    //Excel工作目录
    static string excelFile = Application.dataPath + "/Excel/";
    //输出目录
    static string csFile = Application.dataPath + "/Scripts/CSharp/";
    ...
    static void ToLua(string sheetName, string[,] data)
    {
        ...
        for (int j = 3; j < data.GetLength(0); j++)//行
        {
            ...
            {
                sb.Append(data[2, i] + " = ");
                sb.Append(TypeStringLua(data[1, i], data[j, i]) + ", ");
            }
